...
class TestDataValidatorComprehensive(unittest.TestCase):
    ...
    def test_validate_empty_data(self):
        """测试空数据验证"""
        # 测试空字典
        is_valid, error_msg = self.validator.validate_movie_data({})
        self.assertFalse(is_valid)
        self.assertIn("缺少必需字段", error_msg)
        
        # 测试None
        is_valid, error_msg = self.validator.validate_movie_data(None)
        self.assertFalse(is_valid)
        self.assertIn("数据不能为空", error_msg)
...
class TestAutoOptimizerComprehensive(unittest.TestCase):
    ...
    def test_apply_valid_optimization_actions(self):
        ...
        plan = {
            'should_optimize': True,
            'actions': [
                {'type': 'enable_retry', 'reason': '高失败率'},
                {'type': 'enable_bloom_filter', 'reason': '高重复率'},
                {'type': 'adjust_concurrency', 'value': 10, 'reason': '性能优化'}
            ]
        }
        ...
        self.assertEqual(self.optimizer.config['concurrency'], 10)
    ...
    def test_optimization_with_high_metrics(self):
        ...
        self.assertIn('adjust_concurrency', action_types)
