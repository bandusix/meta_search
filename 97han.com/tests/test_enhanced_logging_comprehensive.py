...
class TestEnhancedLoggerComprehensive(unittest.TestCase):
    ...
    def test_auto_optimize_with_data(self):
        """测试自动优化逻辑"""
        # 添加足够的性能数据
        for i in range(20):
            self.logger.add_performance_data(3.0, i % 5 == 0, i % 10 == 0, 1)
        
        # 手动调用自动优化
        self.logger._auto_optimize()
...
class TestDataValidatorComprehensive(unittest.TestCase):
    ...
    def test_content_duplicate_detection(self):
        ...
        # 验证第一个数据
        is_valid1, _ = self.validator.validate_movie_data(data1)
        self.assertTrue(is_valid1)
        
        # 验证第二个数据（应该因为内容重复而失败）
        is_valid2, error_msg2 = self.validator.validate_movie_data(data2)
        self.assertFalse(is_valid2)
        self.assertIn("重复内容哈希", error_msg2)
...
class TestAutoOptimizerComprehensive(unittest.TestCase):
    ...
    def test_monitoring_thread(self):
        ...
        # 添加一些性能数据
        for i in range(15):
            self.optimizer.add_performance_data(1.0 + i * 0.1, False, False, 1)
